...
def read_users(
    ...
    offset: int = Query(default=0, ge=0, le=settings.list_max_offset),
) -> list[UserRead]:
    require_role(current_user, UserRole.SUPERVISOR, UserRole.TECHNICAL_ADMIN)
    if current_user.role == UserRole.SUPERVISOR:
        if role not in {None, UserRole.ENGINEER}:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Supervisor directory access is limited to engineers",
            )
