...
@pytest.mark.anyio
async def test_request_body_limit_rejects_stream_without_content_length(
...
def test_asset_listing_respects_limit_parameter(
    ...
    supervisor = make_user(
        "supervisor-limit-assets@example.com", UserRole.SUPERVISOR, "ChangeMe123!"
    )
...
def test_report_summary_respects_limit_parameter(
    ...
    supervisor = make_user(
        "supervisor-limit-report@example.com", UserRole.SUPERVISOR, "ChangeMe123!"
    )
    engineer = make_user(
        "engineer-limit-report@example.com", UserRole.ENGINEER, "ChangeMe123!"
    )
...
def test_huge_pagination_offsets_are_rejected(
    client: TestClient,
    make_user: Callable[[str, UserRole, str], User],
    login_as: Callable[[str, str], TokenResponseData],
) -> None:
    supervisor = make_user(
        "supervisor-offset-p5@example.com", UserRole.SUPERVISOR, "ChangeMe123!"
    )
    tokens = login_as(supervisor.email, "ChangeMe123!")
    headers = {"Authorization": f"Bearer {tokens['access_token']}"}

    for path in (
        "/api/v1/assets?limit=1&offset=10001",
        "/api/v1/maintenance-requests?limit=1&offset=10001",
        "/api/v1/users?limit=1&offset=10001",
        "/api/v1/reports/maintenance-summary?limit=1&offset=10001",
    ):
        response = client.get(path, headers=headers)
        assert response.status_code == 422


def test_production_rejects_debug_and_placeholder_bootstrap_passwords() -> None:
    with pytest.raises(ValidationError):
        Settings(
            secret_key="x" * 32,
            bootstrap_admin_password="RealAdminPassword123!",
            bootstrap_staff_password="RealStaffPassword123!",
            bootstrap_user_password="RealUserPassword123!",
