...
def create_user(session: Session, payload: UserCreate, role: UserRole = UserRole.ENGINEER) -> User:
...
def build_token_pair(session: Session, user: User, context: RequestContext) -> TokenPairResponse:
...
def _get_refresh_token_record(session: Session, raw_refresh_token: str) -> RefreshToken | None:
...
def rotate_refresh_token(
    ...
    expires_at = _normalize_utc(token_record.expires_at) if token_record is not None else None
    revoked_at = _normalize_utc(token_record.revoked_at) if token_record is not None else None

    if token_record is None or revoked_at is not None or expires_at is None or expires_at <= now:
        raise HTTPException(
