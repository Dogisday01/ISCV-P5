...
def test_engineer_cannot_view_unrelated_request(
    ...
    supervisor = make_user("supervisor1@example.com", UserRole.SUPERVISOR, "ChangeMe123!")
    requester = make_user("engineer-owner@example.com", UserRole.ENGINEER, "ChangeMe123!")
    outsider = make_user("engineer-outsider@example.com", UserRole.ENGINEER, "ChangeMe123!")
    asset = create_asset(
        db_session,
        supervisor,
        AssetCreate(
...
def test_assets_require_authentication(
    ...
    supervisor = make_user("supervisor-assets@example.com", UserRole.SUPERVISOR, "ChangeMe123!")
    asset = create_asset(
        db_session,
        supervisor,
        AssetCreate(
...
def test_supervisor_assigns_and_engineer_completes_request(
    ...
    supervisor = make_user("supervisor2@example.com", UserRole.SUPERVISOR, "ChangeMe123!")
    ...
    assert assign_response.json()["internal_notes"] == "Dispatch during low-load window only."

    outsider = make_user("engineer-status-outsider@example.com", UserRole.ENGINEER, "ChangeMe123!")
    outsider_tokens = login_as(outsider.email, "ChangeMe123!")
...
def test_invalid_uuid_inputs_are_rejected(
    ...
    supervisor = make_user("supervisor-uuid@example.com", UserRole.SUPERVISOR, "ChangeMe123!")
...
def test_asset_creation_and_report_export_are_role_restricted(
    ...
    supervisor = make_user("supervisor3@example.com", UserRole.SUPERVISOR, "ChangeMe123!")
    engineer = make_user("engineer-report@example.com", UserRole.ENGINEER, "ChangeMe123!")
    supervisor_tokens = login_as(supervisor.email, "ChangeMe123!")
    engineer_tokens = login_as(engineer.email, "ChangeMe123!")
    ...
    assert supervisor_report_response.status_code == 200, supervisor_report_response.text
    payload = supervisor_report_response.json()
    assert payload["total_requests"] >= 0

    stored_asset = db_session.query(Asset).filter(Asset.asset_tag == "COMP-4001").one_or_none()
